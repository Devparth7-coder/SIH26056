...
async def lifespan(app: FastAPI):
    ...
    init_db()
    # Check if database has initial records; if not, seed with 35-day backtest
    ...
    if not row or row["cnt"] == 0:
        ...
        # Train initial ML ensemble
        train_nowcast_model()

    # Launch background autonomous worker daemon
    worker_daemon.start()
    yield
    # Shutdown worker daemon
    worker_daemon.stop()
